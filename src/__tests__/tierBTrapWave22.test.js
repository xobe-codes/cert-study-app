...
import { TIER_B_TRAP_WAVE22_PATCHES } from '../data/tierBTrapWave22Patches.js'
...
describe('tierBTrapWave22', () => {
  it('covers all 53 objectives with one trap each', () => {
    expect(Object.keys(TIER_B_TRAP_WAVE22_PATCHES)).toHaveLength(53)
    for (const obj of ALL_OBJECTIVES) {
      const patch = TIER_B_TRAP_WAVE22_PATCHES[obj.id]
      expect(patch?.examTraps?.length).toBe(1)
      expect(patch.examTraps[0].id).toBe(`${obj.id}-w22-t1`)
    }
  })

  it('merges into curated traps and raises avg beyond 12', () => {
    ...
    expect(enriched.examTraps?.some(t => t.id === '3.1-w22-t1')).toBe(true)
  })
})
