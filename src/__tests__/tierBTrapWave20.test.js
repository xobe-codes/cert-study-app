...
import { TIER_B_TRAP_WAVE20_PATCHES } from '../data/tierBTrapWave20Patches.js'
...
describe('tierBTrapWave20', () => {
  it('covers all 53 objectives with one trap each', () => {
    expect(Object.keys(TIER_B_TRAP_WAVE20_PATCHES)).toHaveLength(53)
    for (const obj of ALL_OBJECTIVES) {
      const patch = TIER_B_TRAP_WAVE20_PATCHES[obj.id]
      expect(patch?.examTraps?.length).toBe(1)
      expect(patch.examTraps[0].id).toBe(`${obj.id}-w20-t1`)
    }
  })

  it('merges into curated traps and raises avg beyond 10', () => {
    ...
    expect(enriched.examTraps?.some(t => t.id === '3.1-w20-t1')).toBe(true)
  })
})
