...
import { isBannedMechanismLanguage, isTemplateWhyWrongHere } from '../src/answerReview/answerReviewQuality.js'
...
for (const path of walkJsonFiles(CLEAN_ROOT)) {
  ...
  for (const q of data.questions || []) {
    for (const item of q.answerReview?.incorrect || []) {
      if (isTemplateWhyWrongHere(item.whyWrongHere) || isBannedMechanismLanguage(item.whyWrongHere)) {
        storedTemplates++
      }
    }
    runtimeChecked++
    ...
    // Gate on runtime-applied reviews (learners never see raw stored templates).
    errors.push(...scanIncorrect(enriched.answerReview?.incorrect, `${rel} ${qid}`))
  }
}

if (errors.length) {
  console.error(`✗ Mechanism language validation failed (${errors.length} issues on ${runtimeChecked} runtime checks):`)
  errors.slice(0, 40).forEach(e => console.error('  -', e))
  if (errors.length > 40) console.error(`  … and ${errors.length - 40} more`)
  process.exit(1)
}

console.log(`✓ Mechanism language OK — ${runtimeChecked} runtime checks${storedTemplates ? ` · ${storedTemplates} stored templates rebuilt at runtime` : ''}`)
process.exit(0)
