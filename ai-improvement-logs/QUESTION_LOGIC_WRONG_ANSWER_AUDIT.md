...
## 3. Regression risks found (documented, not fixed — out of this run's scope)

1. **Missed-entry writer duplication.** Two divergent code paths persist to the same `STORAGE_KEYS.missed` key: `handleMissed`/`saveMissed` (`src/features/progress/useAppProgress.js:35-41`, `src/storage/appPersistence.js:19-21` — no dedup) vs. `appendMissedEntry` (`src/features/domainPass/domainPassStorage.js:165-172` — dedupes by `questionId + objectiveId`). Both consume the same `buildMissedEntry` shape, so this run's additive `diagnosis` field is unaffected, but the dedup inconsistency is a pre-existing regression risk unrelated to this task. **Not fixed** (would touch duplicate-recording behavior outside this task's scope; flagging per "regression risks" requirement).
2. **Post-grade call drift inside `QuizTab.jsx`.** Four submit handlers (`selectAnswer`, `markUnknown`, `submitMulti`, `submitOrder`, `submitCli`) each hand-roll a different subset of post-grade calls (`recordQuestionHealthSignal`, `recordAnswerOutcome`, `recordEngagement`, `recordMissClearAttempt`, `recordTrapMiss` are not called consistently across all four). No shared fan-out function exists. This matches the routine's "Strongly preferred" (not must-ship) item — "shared post-grade orchestration." **Not refactored this run**: touching all four handlers in the highest-traffic session this close to the stated deadline risks the "preserve all existing functionality" priority for a stretch-tier item. Recommended next-phase work (see implementation report).
