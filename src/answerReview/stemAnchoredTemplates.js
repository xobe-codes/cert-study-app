/**
 * Stem-anchored template bodies for the SADE engine (Stem-Anchored Distractor
 * Engine) — the bulk content-generation logic behind buildStemAnchoredIncorrect.
 * Split out of stemAnchoredDistractor.js and lazy-loaded (see
 * loadStemAnchoredTemplates there) because these two functions alone account
 * for the large majority of that module's size — content that only needs to
 * be resident once a learner reaches a question, not before the app is
 * interactive. Preloaded by cleanQuestionAdapter.preloadCleanBank(), the same
 * way goldAnswerReviewsData.js is.
 */
import { normalize, hookPhrase, isStpPortStateSequence, mentionsBothMacAddresses, impliesDropOrDiscard } from './stemAnchoredDistractor.js'

export function contrastWithCorrect({ wrong, correct, hooks, fact, blob }) {
