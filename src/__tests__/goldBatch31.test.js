...
import { BATCH31_GOLD } from '../answerReview/goldAnswerReviewsBatch31.js'
...
describe('goldAnswerReviewsBatch31', () => {
  it('polishes 40 high-traffic short debriefs', () => {
    expect(Object.keys(BATCH31_GOLD).length).toBe(40)
  })

  it('registers each id in GOLD_ANSWER_REVIEWS', () => {
    for (const id of Object.keys(BATCH31_GOLD)) {
      expect(goldAnswerReviewFor(id), id).toBeTruthy()
      expect(goldAnswerReviewFor(id)).toEqual(BATCH31_GOLD[id])
    }
  })

  it('keeps incorrect explanations stem-specific and non-template', () => {
    for (const [id, gold] of Object.entries(BATCH31_GOLD)) {
