...
export const READING_SUPPLEMENTS = {
  '2.3': shell({
    ...
    reading: {
      ...
      tiers: {
        ...
        intermediate: 'CDP is Cisco-proprietary and on by default; it floods updates every 60 seconds and can be disabled per interface. LLDP is the IEEE 802.1AB standard for multivendor environments. Both let you troubleshoot cabling and document topology by showing each neighbor’s detail.',
        examReady: 'CDP is Cisco’s proprietary L2 discovery protocol; LLDP is the vendor-neutral IEEE standard. Both reveal a neighbor’s device ID, IP, platform, and local/remote port — but only for directly connected devices, since neither is routable.',
      },
      ...
      keyPoints: ['CDP: Cisco proprietary, enabled by default, 60s updates; disable per-interface with `no cdp enable`.', 'LLDP: IEEE 802.1AB, `lldp run` to enable.', '`show cdp neighbors detail` / `show lldp neighbors detail` show connected port mapping.', 'Disable CDP on untrusted edge ports for security.'],
  ...
  '2.4': shell({
    ...
    reading: {
      ...
      tiers: {
        beginner: 'EtherChannel combines multiple physical switch links into one faster logical link so STP does not block the extras.\n\nEach physical member keeps forwarding traffic, but the switch and STP see only one bundled interface.',
        intermediate: 'Each physical interface joins the bundle with a matching channel-group command, then VLANs and IP addressing are assigned to the resulting Port-channel interface, not the members. LACP is the standards-based negotiation option and both sides must agree on active or passive mode.\n\nLoad balancing spreads traffic across member links using a hash of MAC or IP address, not a simple round-robin.',
        examReady: 'EtherChannel bundles 2–8 links into one Port-channel (Po) interface. LACP negotiates the bundle (standards-based, 802.3ad); PAgP is the older Cisco-proprietary equivalent.\n\nAll members must match on speed, duplex, and VLAN/trunk mode or the bundle will not form. IP and VLAN configuration goes on the Port-channel interface, never on individual members.',
      },
      ...
      keyPoints: ['2–8 member links per channel.', 'LACP: `channel-group <n> mode active|passive` (802.3ad, standards-based).', 'PAgP: `channel-group <n> mode desirable|auto` (Cisco-proprietary).', 'Configure SVI/IP on `interface Port-channel <n>`, not individual members.', 'Mismatched member settings prevent the bundle from forming.', 'Verify with `show etherchannel summary` and `show interfaces port-channel <n>`.'],
