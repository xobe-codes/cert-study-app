...
import { recordAnswerOutcome } from '../features/study/answerOutcome.js'
...
export default function TopicFocusSession({ config, onBack, onMissed, onDone }) {
  ...
  function answer(idx) {
    ...
    recordAnswerOutcome({ objectiveId: current.objectiveId, questionId: current.id, correct, selectedIndex: idx, surface: 'topic_focus' }).catch(() => {})
    if (!correct) {
  ...
  function submitOrder() {
    ...
    recordQuizResult(current.objectiveId, current.id, { correct })
    recordEngagement?.(current.objectiveId, { kind: ENGAGEMENT_KINDS.TOPIC_FOCUS, correct: correct ? 1 : 0, total: 1 })
    recordAnswerOutcome({ objectiveId: current.objectiveId, questionId: current.id, correct, responseType: 'ordering', surface: 'topic_focus' }).catch(() => {})
    if (!correct) {
  ...
  function submitCli() {
    ...
    recordQuizResult(current.objectiveId, current.id, { correct })
    recordEngagement?.(current.objectiveId, { kind: ENGAGEMENT_KINDS.TOPIC_FOCUS, correct: correct ? 1 : 0, total: 1 })
    recordAnswerOutcome({ objectiveId: current.objectiveId, questionId: current.id, correct, responseType: 'cli', surface: 'topic_focus' }).catch(() => {})
    if (!correct) {
