...
const SADE_AUDIT_JSON = join(ROOT, 'data', 'sade-health-audit.json')
...
function main() {
  ...
  for (const { objectiveId, q } of collectQuestions()) {
    ...
    if (!q.answerReview) {
      ...
      sadeAudits.push({ ...audit, objectiveId })
      const { status, reasons } = sadeHealthPatch(audit)
      if (status && reasons.length) {
        const prev = entries[q.id]
        const nextStatus = status === 'quarantined' || prev?.status === 'quarantined'
          ? 'quarantined'
          : status
        upsertEntry(entries, q.id, objectiveId, { status: nextStatus, reasons })
      }
    }
  ...
  const payload = {
    ...
    sadeQuarantineMin: SADE_QUARANTINE_MIN,
    sadeNeedsFixMin: SADE_NEEDS_FIX_MIN,
    entries,
    excludedIds: [...excluded].sort(),
  }
  writeFileSync(OUT_JSON, `${JSON.stringify(payload, null, 2)}\n`)

  const sadeSummary = summarizeSadeAudits(sadeAudits)
  const sadePayload = {
    generatedAt: payload.generatedAt,
    thresholds: {
      quarantineMin: SADE_QUARANTINE_MIN,
      needsFixMin: SADE_NEEDS_FIX_MIN,
    },
    summary: sadeSummary,
    quarantinedBySade: sadeAudits
      .filter(a => a.min < SADE_QUARANTINE_MIN || a.fallbackCount > 0)
      .map(a => a.id),
    needsFixBySade: sadeAudits
      .filter(a => a.min < SADE_NEEDS_FIX_MIN && a.min >= SADE_QUARANTINE_MIN)
      .map(a => a.id),
  }
  writeFileSync(SADE_AUDIT_JSON, `${JSON.stringify(sadePayload, null, 2)}\n`)
...
`
  ...
  console.log(`  SADE audit — quarantine candidates: ${sadeSummary.quarantineCandidates}, needs_fix: ${sadeSummary.needsFixCandidates}`)
  console.log(`  → ${SADE_AUDIT_JSON.replace(`${ROOT}/`, '')}`)
}
