...
import { TIER_B_TRAP_WAVE23_PATCHES } from '../data/tierBTrapWave23Patches.js'
...
describe('tierBTrapWave23', () => {
  it('covers all 53 objectives with one trap each', () => {
    expect(Object.keys(TIER_B_TRAP_WAVE23_PATCHES)).toHaveLength(53)
    for (const obj of ALL_OBJECTIVES) {
      const patch = TIER_B_TRAP_WAVE23_PATCHES[obj.id]
      expect(patch?.examTraps?.length).toBe(1)
      expect(patch.examTraps[0].id).toBe(`${obj.id}-w23-t1`)
    }
  })

  it('merges into curated traps and raises avg beyond 13', () => {
    ...
    expect(enriched.examTraps?.some(t => t.id === '3.1-w23-t1')).toBe(true)
  })
})
